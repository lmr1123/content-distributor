...
async function test() {
  ...
  // 导航到发布页面
  await page.goto('https://creator.xiaohongshu.com/publish/publish?target=article', { waitUntil: 'domcontentloaded', timeout: 30000 }).catch(() => {});
  ...
  if (titleInput) {
    await titleInput.fill('测试标题 - 图片粘贴测试');
    console.log('✓ 标题已填充');
  }
  ...
  if (editor) {
    ...
    // 输入文字
    await page.keyboard.type('这是测试正文内容，接下来会粘贴图片。');
    console.log('✓ 正文已填充');
  }
  ...
  // 模拟粘贴事件
  const pasteResult = await page.evaluate((base64Img) => {
  ...
  console.log('编辑器中有图片:', hasImage);

  // 截图
  await page.screenshot({ path: '/tmp/xhs-long-article-test.png', fullPage: false });
  console.log('截图: /tmp/xhs-long-article-test.png');
