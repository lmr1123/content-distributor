/**
 * 小红书写长文 - 通过模拟粘贴上传图片
 */

const { chromium } = require('playwright');
const fs = require('fs');
const path = require('path');

async function test() {
  console.log('连接到 Chrome...');
  const browser = await chromium.connectOverCDP('http://127.0.0.1:9222');
  const ctx = browser.contexts()[0];
  const page = ctx.pages().find(p => p.url().includes('xiaohongshu')) || ctx.pages()[0];

  if (!page) {
    console.log('未找到页面');
    await browser.close();
    return;
  }

  page.on('dialog', async dialog => {
    try { await dialog.dismiss(); } catch (e) {}
  });

  console.log('当前页面:', page.url());

  // 导航到写长文编辑器
  await page.goto('https://creator.xiaohongshu.com/publish/publish?target=article', { waitUntil: 'networkidle' });
  await page.waitForTimeout(3000);

  const tabs = await page.$$('.creator-tab');
  for (const tab of tabs) {
    const text = await tab.textContent();
    if (text?.includes('写长文')) {
      await tab.evaluate(el => el.click());
      await page.waitForTimeout(2000);
      break;
    }
  }

  const createBtn = await page.locator('button:has-text("新的创作")').first();
  if (await createBtn.count() > 0) {
    await createBtn.click({ force: true });
    await page.waitForTimeout(4000);
  }

  // 查找隐藏的 file input
  console.log('\n========================================');
  console.log('查找隐藏的 file input');
  console.log('========================================');

  // 有时候 file input 是隐藏的，需要用 evaluate 查找
  const hiddenInputs = await page.evaluate(() => {
    const inputs = document.querySelectorAll('input');
    return Array.from(inputs).map(input => ({
      type: input.type,
      accept: input.accept,
      className: input.className,
      style: input.getAttribute('style'),
      hidden: input.hidden || input.style.display === 'none' || input.offsetParent === null
    }));
  });

  console.log('所有 input:');
  hiddenInputs.forEach((inp, i) => {
    console.log(`  ${i + 1}. type="${inp.type}" accept="${inp.accept}" hidden=${inp.hidden}`);
  });

  // 尝试通过 CDP 注入文件到页面
  console.log('\n========================================');
  console.log('尝试使用 CDP 上传文件');
  console.log('========================================');

  // 创建测试图片
  const testImagePath = '/tmp/test-image.jpg';
  const minJpeg = Buffer.from([
    
  ]);
  fs.writeFileSync(testImagePath, minJpeg);
  console.log('创建测试图片:', testImagePath);

  // 查找并注入到隐藏的 file input
  const cdpSession = await page.context().newCDPSession(page);

  // 获取所有 input[type=file] 元素（包括隐藏的）
  const fileInputHandle = await page.evaluateHandle(() => {
    const inputs = document.querySelectorAll('input[type="file"]');
    return inputs.length > 0 ? inputs[0] : null;
  });

  if (fileInputHandle) {
    console.log('找到 file input，尝试上传...');
    try {
      await fileInputHandle.setInputFiles(testImagePath);
      console.log('✓ 文件上传成功！');

      await page.waitForTimeout(3000);
      await page.screenshot({ path: '/tmp/xhs-image-uploaded.png', fullPage: false });
      console.log('截图: /tmp/xhs-image-uploaded.png');
    } catch (e) {
      console.log('上传失败:', e.message);
    }
  } else {
    console.log('未找到 file input');
  }

  await browser.close();
}

test().catch(console.error);
